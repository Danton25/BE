import java.util.*;
import java.io.*;
import java.nio.*;//for hasremaining function
...
("InputFile")); // InputFile is file which holds multiplier
writer.write(input);
writer.close();
...
char ch;
while (bb.hasRemaining()) {//tells whether there are any elements between the current position and the limit.
ch = (char) bb.get();
num1 = num1 + ch;
}
...
longMult(multiplier1,multiplier2)); //calling longmult function
}
}
